...
and [<AbstractClass>] Material() = 
    ...
    abstract Scatter : ray:Ray * hit:RayHit -> ValueOption<MaterialHit>
    abstract RandomUnitInSphere : unit -> Vector3
    default x.RandomUnitInSphere() =
        let newP() = Vector3.Create(2.f * nextDouble() - 1.f)
        let mutable p = newP()
...
type Surface = 
     abstract IsHit : ray:Ray * tMin:float32 * tMax:float32 -> ValueOption<RayHit>
...
let metal (albedo:Vector3, fuzz) =
    { new Material() with 
        member x.Scatter(ray:Ray, hit:RayHit) = 
            ...
            if length > 0.f 
            then ValueSome { attenuation = albedo; scatteredRay = scattered }
            else ValueNone
    } 
...
let diffuse (albedo:Vector3) =          
    { new Material() with 
        member x.Scatter(ray:Ray, hit:RayHit) = 
            ...
            ValueSome { attenuation = albedo; scatteredRay = scattered }
    }             
...
type Scene(items:seq<Surface>) =
    ...
    static member Random() = 
        ...
        objects.Add(sphere (Vector3.Create(0.f,-1000.f, 0.f), 1000.f, diffuse(Vector3.Create(0.5f,0.5f, 0.5f))))
        let rng = [-11 .. 1 .. 11]
        let mutable i = 1;
        for a in  rng do
          for b in rng do
            let a = float32 a 
            let b = float32 b
            let center = Vector3.Create(a-0.9f*nextDouble(), 0.2f, b+0.9f*nextDouble())
            let mat =
                if (nextDouble() > 0.5f)
                then  metal (Vector3.Create(0.5f * (1.f + nextDouble()),0.5f * ( 1.f + nextDouble()),0.5f * (1.f + nextDouble())), 0.5f * nextDouble())
                else  diffuse (Vector3.Create(nextDouble() * nextDouble(),nextDouble() * nextDouble(),nextDouble() * nextDouble()))
                

            objects.Add(sphere (center, 0.2f, mat))
    ...
    let rec trace(ray:Ray, surface:Surface, depth:int) =
        ...
        | ValueSome hit -> 
            ...
            then 
                ...
                | ValueSome m -> m.attenuation * (trace(m.scatteredRay, surface, depth + 1))
    ...
    member x.Render(camera:Camera, scene) = 
        ...
        for y in rngY do 
            for x in rngX do
                ...
                for s in samples do 
                    let i = ((float32 x) + nextDouble()) / (float32 file.X)
                    let j = ((float32 y) + nextDouble()) / (float32 file.Y)
